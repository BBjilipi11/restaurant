      ...
      <div className="w-full h-fit bg-white flex flex-col md:py-24 py-8 md:relative">
        <div className="w-full h-full flex md:flex-row flex-col items-start justify-between gap-8 md:px-32 px-8">
          ...
          <div className="w-full md:w-7/12 h-full flex flex-col justify-center items-start gap-[18px]">
            <div className="w-full flex gap-4 border-b-[1px] border-b-[#2563eb]">
              {buttonNames.map((button) => (
                <button
                  key={button.id}
                  className={`font-roboto text-[14px] font-normal leading-[14px] text-[#181818] px-4 py-2 transition-all duration-300 ease-in-out scale-100 hover:scale-110 active:scale-90 ${
                    activeButton === button.id
                      ? activeButtonStyle.active
                      : activeButtonStyle.inactive
                  }`}
            ...
            <h5 className="md:text-[62px] text-[40px] font-bebas font-bold md:leading-[62px] leading-[48px] text-[#181818] transition-all duration-300 ease-in-out">
              {buttonNames[activeButton - 1].title}
            </h5>
            <p className="font-roboto text-base font-normal text-justify text-[#333333] transition-all duration-300 ease-in-out">
            ...
            <div className="flex gap-8 transition-all duration-300 ease-in-out">
              ...
              <button className="md:flex hidden items-center justify-start gap-2 font-roboto text-[18px] font-bold text-black">
                <MdOutlineWifiCalling3 className="text-blue-600 text-[20px]" />
          ...
          {aboutItems.map((item) => (
            ...
            >
              ...
              <div className="flex flex-col items-start justify-center gap-2">
                <h5 className="text-[30px] font-bebas font-bold leading-[30px] text-black">
                  {item.title}
                </h5>
                <p className="font-roboto text-[20px] font-normal leading-[24px] text-black">
                  {item.description}
                </p>
              </div>
            </div>
        ...
        <img
          src={sideImage}
          alt="side"
          className="absolute top-[340px] right-0 md:block hidden"
        />
      </div>
    </section>
  );
